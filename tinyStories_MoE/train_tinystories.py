...
def parse_args() -> TrainConfig:
    ...
    p.add_argument('--early_stop_warmup', type=int, default=400)
    p.add_argument('--max_time_min', type=int, default=None)
...
def save_checkpoint(out_dir: str, model: GPTMoE, enc_name: str, cfg: TrainConfig, step: int):
    ...
    meta = {
        ...
        'arch': {
            ...
            'seq_len': cfg.seq_len,
            'd_ff': cfg.d_ff,
            'moe_layer_index': cfg.moe_layer_index,
            'moe_layers': cfg.moe_layers,
            'moe_all_layers': cfg.moe_all_layers,
...
def main():
    ...
    scaler = torch.amp.GradScaler(device.type, enabled=(device.type == 'cuda' and not cfg.bf16))
    ...
    def run_eval(max_batches: int):
        ...
        with torch.no_grad():
            for i, (xe, ye) in enumerate(val_dl):
                ...
                with torch.amp.autocast(device.type, dtype=amp_dtype, enabled=(device.type in ('cuda', 'cpu'))):
                    _, ce, _ = model(xe, ye)
                total += float(ce.item())
                n += 1
        model.train()
    ...
    while step < cfg.steps and not stop_training:
        for x, y in train_dl:
            ...
            if step % 20 == 0:
                print(
                    f"step {step:5d} | loss {loss.item():.4f} | aux {aux.item():.4f} | "
                    f"val {val_loss:.4f} ppl {val_ppl:.2f} | {toks_sec:.0f} tok/s | mem {gpu_mem_mb:.0f}MB"
                )

            if csv_f is not None:
                csv_f.write(
                    f"{step},{loss.item():.6f},{aux.item():.6f},{val_loss:.6f},{val_ppl:.6f},{toks_sec:.2f},{gpu_mem_mb:.1f}\n"
                )
                csv_f.flush()
